"""PyTorch full-parameter fine-tune: gen_2 → depth 6 labels, unfrozen FT."""
...
recs = load_bin('models/nnue_trained/sp_d6.bin')
n = len(recs); split = int(n * 0.9)
tr = recs[:split]; va = recs[split:]
print(f'{len(tr)}/{len(va)}', flush=True)
...
for ep in range(500):
    rnd.shuffle(tr); tl = 0.0; tc = 0
    for i in range(0, len(tr), 1024):
        b = tr[i:i+1024]; B = len(b)
        sf = torch.zeros(B, 360, device=DEVICE)
        nf = torch.zeros(B, 360, device=DEVICE)
        dt = torch.zeros(B, 66, device=DEVICE)
        lb = torch.zeros(B, device=DEVICE)
        for j, (sp, de, la, s) in enumerate(b):
            for f in sp:
                if f < 0 or f >= 360: continue
                p1 = f < P1_CUTOFF
                a = (s == 0 and p1) or (s == 1 and not p1)
                if a: sf[j, f] = 1.0
                else: nf[j, f] = 1.0
    ...
    with torch.no_grad():
        for i in range(0, len(va), 1024):
            ...
            h2 = torch.relu(model.fc2(h1))
            out = torch.tanh(model.fc3(h2)).squeeze(1)
            vl += nn.MSELoss()(out, lb).item() * B; vc += B
    vl /= vc
    scheduler.step(vl)
    if vl < best_val: best_val = vl; best_sd = model.state_dict().copy()
    if ep % 25 == 0:
        lr_now = optimizer.param_groups[0]['lr']
        print(f'ep {ep+1:>3d}/500  train={tl/tc:.4f}  val={vl:.4f}  LR={lr_now:.1e}', flush=True)

model.load_state_dict(best_sd)
torch.save({'model_state': model.state_dict()}, 'models/nnue_trained/pt_ft_full.pt')
...
print(f'FT full (unfrozen) vs Random: {w}/100 ({w}%)', flush=True)
