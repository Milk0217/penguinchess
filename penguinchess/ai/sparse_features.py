...
PHASE_PLACEMENT = 0.0
PHASE_MOVEMENT = 0.5
PHASE_OVER = 0.0  # game over treated as placement for phase feature
...
def _decode_piece_hex(idx: int) -> tuple[int, int]:
    """Decode sparse feature index → (piece_idx, hex_idx)."""
    return idx // HEX_COUNT, idx % HEX_COUNT


def extract_sparse(core: PenguinChessCore) -> list[int]:
    """
    Extract sparse binary features from game state.
    
    Returns list of active feature indices (max 6).
    Each index represents a (piece_idx, hex_idx) pairing.
    """
    features = []
...
def extract_dense(core: PenguinChessCore) -> np.ndarray:
    """
    Extract dense features from game state.
    
    Returns (DENSE_DIM,) array:
      hex_values[0:60] — normalized hex value
      meta[0:6] — scores, phase, alive counts, steps
    """
    hex_values = np.zeros(HEX_COUNT, dtype=np.float32)
    for i, h in enumerate(core.hexes):
        # Normalize: 0 = eliminated, 1/3, 2/3, 1.0
        if h.state in ('used', 'eliminated'):
            hex_values[i] = 0.0
        elif h.state == 'occupied':
            hex_values[i] = 0.0  # occupied hex has 0 points
        else:
            hex_values[i] = h.points / 3.0  # active → 1/3, 2/3, or 1.0
...
def state_to_features(core: PenguinChessCore) -> tuple[list[int], np.ndarray]:
    """Convenience: returns (sparse_indices, dense_array)."""
